...
<script type="text/javascript">
  ...
  const upColor2 = '#ec0000';
  const upBorderColor2 = '#8A0000';
  const downColor2 = '#00da3c';
  const downBorderColor2 = '#008F28';
  // Each item: open，close，lowest，highest
  const data2 = splitData([
    <?php
    ...
    $sql = "SELECT * FROM twa  WHERE stock LIKE '%" . $input . "%' ";
  ...
  function calculateMA(dayCount) {
    var result = [];
    for (var i = 0, len = data2.values.length; i < len; i++) {
      if (i < dayCount) {
        result.push('-');
        continue;
      }
